...
import { sendNewUserEmail } from './lib/ses.utils';
...
function convertOldSettings(user: DBUser) { // temporary convert function
  try {
    // tslint:disable-next-line:no-any
    const oldSettings = (user as any).settings as UserSettingsView;
    if (oldSettings) {
      const convertedItems: Array<UsiUnion> = [];
      convertedItems.push(...oldSettings.bodyWeightPoints.map((bwp) => ({
        kind: 'bodyWeightPoint',
        id: bwp.date,
        value: bwp,
      } as UsiBodyWeightPoint)));

      convertedItems.push(...oldSettings.favorites.map((fav) => ({
        kind: 'favorite',
        id: fav,
        value: fav,
      } as UsiFavorite)));

      convertedItems.push(...oldSettings.meals.map((meal) => ({
        kind: 'meal',
        id: String(meal),
        value: meal,
      } as UsiMeal)));

      convertedItems.push(...oldSettings.prices.map((pi) => ({
        kind: 'price',
        id: pi.productId,
        value: pi,
      } as UsiPrice)));
...
export const handler = createHandler<RemoteSettingsSimple>({required: true}, {
  post: async (req) => {
